...
class TreeTrainer(BaseTrainer):
    ...
    def get_onnx_model(self, lgb_model):
        ...
        onnx_model = convert(lgb_model, initial_types=initial_types, target_opset=8)
        return onnx_model
    ...
    def train_gbdt(
        ...
        if hp_tune:
            ...
            # train the model
            gbdt_model = base_estimator.fit(x_train, y_train)

        # predict on testing and validation set
        if (
            task == "classification" or task == "classification-regression"
    ...
    def train_lmdt(self, task, directory, hp_tune=None):
        ...
        if hp_tune:
            ...
            # Create the GridSearchCV object
            grid = GridSearchCV(estimator=lmdt, param_grid=param_grid, cv=3, verbose=1)
